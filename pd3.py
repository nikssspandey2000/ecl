...
# Build quarter to fiscal year mapping
...
# ===================================================================
# NEW SECTION: Yearly average of annual probabilities (4th power)
# ===================================================================

# Prepare data for yearly averaging
yearly_avg_data = annual_probs[annual_probs['Quarter'] != 'All Quarters'].copy()
yearly_avg_data['Fiscal Year'] = yearly_avg_data['Quarter'].map(quarter_to_fiscal)

# Get all transition column names
trans_cols = [f'Stage {s1} to {s2}' for s1 in stages for s2 in stages]

# Group by Fiscal Year and Product Type to calculate averages
yearly_avg_rows = []
if not yearly_avg_data.empty:
    grouped = yearly_avg_data.groupby(['Fiscal Year', 'Product Type'])
    
    for (fiscal_year, product_type), group in grouped:
        # Calculate average for each transition
        avg_probs = group[trans_cols].mean().to_dict()
        
        # Get quarters included in this average
        quarters_included = sorted(group['Quarter'].unique())
        
        yearly_avg_rows.append({
            'Fiscal Year': fiscal_year,
            'Product Type': product_type,
            **avg_probs,
            'Quarters Included': ", ".join(quarters_included),
            'Quarter Count': len(quarters_included)
        })

# Create DataFrame
yearly_avg_annual_probs = pd.DataFrame(yearly_avg_rows)
...
with pd.ExcelWriter('transition_matrices.xlsx') as writer:
    ...
    # NEW SHEET: Yearly average annual probabilities
    yearly_avg_annual_probs.to_excel(
        writer, 
        sheet_name='Yearly Average Annual Probabilities', 
        index=False
    )
        ...
        if sheet_name in ['Transition Amounts']:
            ...
            for col in range(2, len(transitions) + 2):
                ws.set_column(col, col, 18, writer.book.add_format({'num_format': num_format}))
                           ...
                           'Annual Verification', 'Yearly Average Annual Probabilities']:
            # Format as percentages
            start_col = 2
            if sheet_name == 'Yearly Average Annual Probabilities':
                # Account for Fiscal Year, Product Type, Quarters Included, Quarter Count
                num_prob_cols = len(trans_cols)
                for col in range(2, 2 + num_prob_cols):
                    ws.set_column(col, col, 18, writer.book.add_format({'num_format': pct_format}))
            else:
                # For other sheets, format specific probability columns
...
print("NEW SHEET ADDED: 'Yearly Average Annual Probabilities'")
